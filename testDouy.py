...
def start():
    url = "https://www.4493.com/xingganmote/64944/1.htm";
    i = 0 ;
    while( url !='') :
        i = i + 1;
...
def readHtml(m,num):
    ...
    myItems = re.findall(r'<div class="picsbox picsboxcenter">(.*)<script type="text/javascript">', m,re.S)  # <a href="(.*)">(.*)</a>
    print myItems[0]
    y=0
    hrefList=re.findall(r"(?<=href=\").+?(?=\")|(?<=href=\').+?(?=\')" ,myItems[y])
    print hrefList[1]
    hrefstr =  re.findall("javascript:gotourl('(.*)');",hrefList[1])
    ...
    # }
    img = requests.get(picStr[0], headers=None, allow_redirects=False,timeout=10).text
    print img
