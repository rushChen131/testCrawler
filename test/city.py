...
class copyPic:
    def __init__(self):
        ...
        self.ranNum = 1000
        self.path ='D:/jd/'
        self.func = execjs.compile(open(r"md5.js").read().decode("utf-8"))
        # self.path ='/home/chen/jiandan/'
        # db = connDB.getDbConn();
    ...
    def saveFile(self,url, num):
        try:
            ...
            print e.reason
        # myItems = re.findall('<ol class="commentlist" style="list-style-type: none;">(.*)</ol>', m,re.S)  # <a href="(.*)">(.*)</a>
        # myItems = re.findall('<ol class="commentlist" style="list-style-type: none;">(.*)</ol>', m,re.S)  # <a href="(.*)">(.*)</a>
        # img = re.findall('<img src="(.*)" />',myItems[0])
        img = re.findall('<span class="img-hash">(.*)</span>',m)
        self.ranNum = self.randomNum()
        for y in range(len(img)):
            func = self.func
            hash = img[y]
            strings = func.call("img",hash)
            picUrls = "http:"+strings
            print picUrls
